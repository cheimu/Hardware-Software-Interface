 ...
 */
int intSize() {
  // address of the second element subtracted by the address
  // of the first element will be the same as size of the 
  // arrray type and since int is only 4 byptes, not big enough 
  // to hold a pointer value. Thus, I casted pointer to an int 
  int intArray[10];
  int * intPtr1;
  int * intPtr2;
 ...
 */
int changeValue() {
  // utilizing pointer arithmetics to locate fifth element in the array
  // then using * to change value at locaiton indicated by computed address
  int intArray[10];
  int * intPtr1;
  intPtr1 = intArray + 5;
...
int withinSameBlock(int *ptr1, int *ptr2) {
  // address of a memory block is typically multiple of 64
  // an adress after being masked off the last six digit 
  // will be the addree of its memory block
  int mask;
  mask = (~0) << 6;
...
int withinArray(int * intArray, int size, int * ptr) {
  // if an address is within the a given array, the given address subtracted
  //  by address of first element in given array and last adress in the given 
  // array is subtractd by address of the address should all be non-zero
  long begin, end, point, sign_begin, sign_end;
  begin = (long) intArray;
  end = ((long) (intArray + size) + (~0));
  point = (long) ptr;
  begin = point + (~begin + 1);
  end =  end + (~point + 1);
  sign_begin = (begin >> 63) & 1;
  sign_end = (end >> 63) & 1;
  return (sign_begin == 0) & (sign_end == 0);     
}
