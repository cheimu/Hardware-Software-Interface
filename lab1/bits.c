...
int addOK(int x, int y) {
  int x_sign = x >> 31;
  int y_sign = y >> 31;
  int sign_req = ~(x_sign ^ y_sign);
  int temp_sign = (x + y) >> 31;

  int result = ~(sign_req & (x_sign ^ temp_sign));
  int mask = 0x01;
  return mask & result;
...
int conditional(int x, int y, int z) {
  int x_sign = ((!!x) << 31) >> 31;
  int x_sign_neg = ((!x) << 31) >> 31;
  return (x_sign & y) | (x_sign_neg & z);
}
 ...
 */
int isPower2(int x) {
  int power_of_2 = !(x & (x+(~0)));
  int not_zero = ~((!x << 31) >> 31);
  int not_negative = !(x >> 31);
  return power_of_2 & not_zero & not_negative;
}
