...
void* mm_malloc (size_t size) {
  ...
  // if there is no enought free space, require more from heap
  if (ptrFreeBlock == NULL) {
     requestMoreSpace(reqSize);
     ptrFreeBlock = searchFreeList(reqSize);
  } 

  // set flag and get size of new allocated free space
  precedingBlockUseTag = ptrFreeBlock->sizeAndTags & TAG_PRECEDING_USED;
  blockSize = SIZE(ptrFreeBlock->sizeAndTags);
  
  // if the free block space we need to use is can be put in required size but smaller
  // than 2 times required size, we simply use this free block and set flag; otherwise
  // to fully utilize space, we need to split into two required sizes  
  if (blockSize - reqSize < MIN_BLOCK_SIZE) {
     // set pointer to point to the flag position of footer
     followingBlock = (BlockInfo*) UNSCALED_POINTER_ADD(ptrFreeBlock, blockSize);
     // set both flags of header and footer
     ...
     // set pointer to extra space that we can save
     BlockInfo* extraFreeBlock = (BlockInfo*)UNSCALED_POINTER_ADD(ptrFreeBlock, reqSize);
     // set flag for extra space header
     extraFreeBlock->sizeAndTags = (blockSize - reqSize) | TAG_PRECEDING_USED;
     // set flag for extra space footer
     *((size_t*) UNSCALED_POINTER_ADD(ptrFreeBlock, blockSize - WORD_SIZE)) = (blockSize - reqSize) | TAG_PRECEDING_USED;
     // insert free block for extra space that don't need
     insertFreeBlock((BlockInfo*) UNSCALED_POINTER_ADD(ptrFreeBlock, reqSize));
  }
  // set that block as used block and remove free one.
  removeFreeBlock(ptrFreeBlock);
  return UNSCALED_POINTER_ADD(ptrFreeBlock, WORD_SIZE);
}

/* Free the block referenced by ptr.                          */ 
/* Find Header, set flag and then find footer and set the flag 
   insert the free block and coalesce                         */
void mm_free (void *ptr) {
  size_t payloadSize;
  BlockInfo * blockInfo;
  BlockInfo * followingBlock;

  //Get the address of Header and get the corresponding information
